...
- Hardware 标签页位于主页与 Docker 之间。它保留 CPU 的受限原始拓扑信息（架构、厂商、核心/线程、频率、缓存与虚拟化）、短时 CPU 使用率细目（含 I/O 等待）、内存/SWAP、系统信息、文件系统/存储卷和物理磁盘。物理磁盘会按已关联的分区或逻辑卷逐行列出分区/格式、已用/总容量和使用率条。文件系统与物理磁盘不强制一对一：LVM、MD RAID、device mapper 和 Btrfs/EXT4 等逻辑栈会列出其已解析的后端物理磁盘。该页只展示经配置的只读采样结果，不读取目录内容、磁盘序列号或 SMART 原始属性。
