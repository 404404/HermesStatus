...
- Hardware 标签页位于主页与 Docker 之间。CPU 信息和 CPU 使用率在桌面端并列：左侧只显示架构、步进、插槽/核心/线程汇总、当前及最低/最高频率和虚拟化，右侧显示同样六行的短时总量、用户态、内核态、I/O 等待、IRQ 与 SoftIRQ；不显示厂商、系列/型号、拓扑、缓存、空闲、Nice 或 Steal。内存/SWAP 与系统信息在桌面端每行三项；物理内存和 Swap 都合并为已用/可用/总量及已用百分比，随后显示空闲内存和其余明细。独立的“文件系统/存储卷”区块已移除；经配置的只读文件系统 probe 仅用于为物理磁盘行关联分区/格式、已用/总容量和使用率条。LVM、MD RAID 与 device mapper 会列出可安全解析的后端物理磁盘；多设备 Btrfs 无法完整枚举成员时明确保留未知关系，绝不伪造单盘归属。该页不读取目录内容、磁盘序列号或 SMART 原始属性。
