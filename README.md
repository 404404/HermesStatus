...
- Hardware 标签页位于主页与 Docker 之间，分为系统信息、文件系统/存储卷和物理磁盘三部分。文件系统与物理磁盘不强制一对一：LVM、MD RAID、device mapper 和 Btrfs/EXT4 等逻辑栈会列出其已解析的后端物理磁盘。该页只展示经配置的只读采样结果，不读取目录内容、磁盘序列号或 SMART 原始属性。
