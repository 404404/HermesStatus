...
- UCG Max has five thermal zones; `lm63` `fan1_input` is a verified hwmon RPM observation. The sensor channel is not proof of a physical fan, so its normalized capability remains `supported=unknown`; `fan1=0` remains an `observed_zero_rpm` value without confirmed physical-fan presentation. The profile declares NVMe capability and does not declare SATA SSD or TF capability; NVMe not observed is not evidence of an absent physical NVMe device.
