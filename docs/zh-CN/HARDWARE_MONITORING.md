...
1. CPU 详情和 CPU 使用率在桌面端各占 1/2；使用率区域以两列展示总量、I/O 等待、用户态和内核态，百分比仅显示在进度条内。不渲染厂商、系列/型号、拓扑、缓存、空闲、Nice、Steal、IRQ 或 SoftIRQ。
2. 内存详情严格转置原有的三列纵向结构：物理内存/活动-非活动/可回收 Slab、Swap/Buffers/Slab、空闲内存/页面缓存、Swap Cache/Dirty-Writeback；四行三列的动态宽度网格在常见 16:9 桌面宽度下保持数值单行显示。
3. 系统信息仅展示操作系统版本、架构和内核。
