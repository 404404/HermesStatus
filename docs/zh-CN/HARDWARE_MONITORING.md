...
1. CPU 详情：架构、厂商、系列/型号/步进、插槽、核心、线程、频率、缓存和虚拟化等受限原始拓扑信息，以及短时 CPU 时间占比。
2. 内存详情：总量、已用、可用、空闲、Buffers、缓存、可回收 Slab、活动/非活动、Dirty/Writeback、Slab 和 Swap 账目。
3. 系统信息：发行版/版本、内核和架构。
4. 文件系统/存储卷：已配置挂载点的容量及解析出的后端物理盘。
5. 物理磁盘：型号、容量、温度、SMART 结果、通电时间和累计计数器；每个关联分区或逻辑卷独占一行，展示分区/格式、已用/总容量和使用率条。
...
Client 基于只读系统元数据构建有上限、可防循环的 block-device graph 来推导关系，不会根据 `dm-0`、`md2` 或厂商特定挂载点名称猜测。它支持普通分区和通用 LVM、MD RAID、device-mapper、Btrfs/EXT4 存储栈。一个文件系统行可对应多块后端磁盘，绝不会被填入臆造的单盘温度或 SMART 结果。
