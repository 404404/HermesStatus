...
1. CPU 详情：桌面端左侧 1/3 逐行展示架构、步进、插槽/核心/线程汇总、当前及最低/最高频率和虚拟化；右侧两个等宽列展示总量、I/O 等待、用户态和内核态，标题、百分比和进度条同行。不渲染厂商、系列/型号、拓扑、缓存、空闲、Nice、Steal、IRQ 或 SoftIRQ。
2. 内存详情：桌面端按列从上到下展示：首列为物理内存和 Swap 的已用/可用/总量及已用百分比、空闲内存、Swap Cache；次列为活动/非活动、Buffers、页面缓存、Dirty/Writeback；最后一列为其余明细。首列标题预留更宽空间。
3. 系统信息：发行版/版本、内核和架构。
