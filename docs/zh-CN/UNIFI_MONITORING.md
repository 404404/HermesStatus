...
UniFi 监控是 HermesStatus 2.5 开发线中 profile 驱动、只读的远端观测域。它有意只覆盖已完成资格验证的 UDW 与 UCG Max console 机型；不是 UniFi controller 集成、设备发现、资产清单、管理 API、远程 shell 或配置通道。
...
- UCG Max 有五个 thermal zone 与 `lm63` diagnostics，但物理 mapping 未知。`fan1=0` 保留为 `observed_zero_rpm`，物理 presence 仍为 unknown。未观察到 NVMe 不能证明物理 NVMe 不存在。

raw thermal zone、hwmon detail、cpuload diagnostics、PWM、未映射 PSU sensor 和不确定 NVMe diagnostics 不进入 V1 UI，也不自动影响健康。
...
UniFi 标签页只呈现 profile、传输/新鲜度、CPU 使用率、CPU 温度、内存、uptime、load 与有界的 fan/PSU/storage capability 状态。它复用浏览器现有的单一 stats 文档和刷新 timer，不会建立独立 polling endpoint。
