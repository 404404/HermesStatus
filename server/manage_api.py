...
WEB_DIR = os.environ.get("WEB_DIR", "/usr/share/nginx/html")
...
def load_stats():
    path = os.path.join(WEB_DIR, "json", "stats.json")
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)


def hermes_config_summaries(profile=None):
    stats = load_stats()
    summaries = []
    for server in stats.get("servers", []):
        hermes = server.get("hermes") if isinstance(server, dict) else {}
        for row in (hermes or {}).get("profiles", []):
            if not isinstance(row, dict):
                continue
            row_profile = str(row.get("profile") or "")
            if profile and row_profile != profile:
                continue
            summary = row.get("config_summary") if isinstance(row.get("config_summary"), dict) else {}
            summaries.append({
                "server": server.get("name") or server.get("host") or "",
                "profile": row_profile,
...
def api_schema():
    return {
        ...
        "endpoints": [
            ...
            {"method": "GET", "path": "/api/hermes/config-summary", "auth": True},
        ...
        try:
            ...
            if path == "/api/config":
                ...
                if method == "PUT":
                    ...
                    return
            if path == "/api/hermes/config-summary" and method == "GET":
                profile = (query.get("profile") or [""])[0]
                self.send_json(200, {"ok": True, "profiles": hermes_config_summaries(profile=profile)})
                return
            if path == "/api/servers":
                if method == "GET":
                    config = load_config()
